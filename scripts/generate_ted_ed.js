...
const BRAND = {
    colors: {
        paper: '#f5f5dc',      // Beige/Cream
        ink: '#1e293b',        // Dark Slate Blue (Ink)
        accent: '#e11d48',     // Rose 600 (TED Red-ish)
        grid: '#94a3b8',       // Slate 400 (Pencil)
        blueprint: '#1e3a8a'   // Blue 900 (Blueprint Background)
    },
    fonts: {
...
function generateTedEdThumbnail() {
    const width = 1280;
    const height = 720;
    const cx = width / 2;
    const cy = height / 2;
    
    // V1: The "Why X Matters" Blueprint
    createSVG({
        filename: 'thumb_v5_ted_ed.svg',
        width, height,
        content: `
            <!-- Background: Blueprint Blue -->
            <rect width="${width}" height="${height}" fill="${BRAND.colors.blueprint}"/>
            ...
            <!-- Grid Pattern -->
             ${DEFS.blueprintGrid(width, height)}
             
            <!-- Central Diagram (The Logo Deconstructed) -->
            <g transform="translate(${cx - 200}, ${cy - 200}) scale(0.4)">
                <!-- Outline Stroke Mode -->
                <path d="${LOGO_Path}" fill="none" stroke="white" stroke-width="4" stroke-dasharray="10 10" opacity="0.6"/>
                <path d="${LOGO_Path}" fill="none" stroke="white" stroke-width="2" transform="translate(20, 20)" opacity="0.3"/>
                
                <!-- Measurement Indicators -->
                <line x1="-50" y1="0" x2="-50" y2="1024" stroke="white" stroke-width="2"/>
                <text x="-120" y="512" fill="white" font-family="monospace" font-size="60" transform="rotate(-90, -120, 512)">1024 UNITS</text>
            </g>
            ...
            <!-- Title Bar (Bottom) -->
            <rect x="0" y="${height - 160}" width="${width}" height="160" fill="${BRAND.colors.paper}"/>
            <rect x="0" y="${height - 160}" width="${width}" height="10" fill="${BRAND.colors.accent}"/>
            
            <!-- Title Text -->
            <text x="60" y="${height - 60}" fill="${BRAND.colors.ink}" font-family="${BRAND.fonts.main}" font-weight="bold" font-size="70">
                The Geometry of Design?
            </text>
            
            <!-- TED-Ed Logo Placeholder -->
            <rect x="${width - 180}" y="${height - 120}" width="140" height="80" rx="10" fill="${BRAND.colors.accent}"/>
            <text x="${width - 110}" y="${height - 70}" text-anchor="middle" fill="white" font-family="${BRAND.fonts.sans}" font-weight="bold" font-size="30">TED-Ed</text>
        `
    });
}

// --- Run ---
console.log('Generating TED-Ed Style Asset...');
generateTedEdThumbnail();
console.log('Done.');
