...
const BRAND = {
    colors: {
        bg: '#f0f0f0',      // Light Grey (Map Base)
        water: '#cbd5e1',   // Slate 300 (Map Water)
        land: '#ffffff',    // White
        highlight: '#fff200', // Vox Yellow
        kenichiHighlight: '#3b82f6', // Electric Blue highlight
...
const DEFS = {
    ...
    sticker: `
        <filter id="sticker">
            ...
            <feDropShadow dx="15" dy="15" stdDeviation="10" flood-color="rgba(0,0,0,0.3)"/>
            <feMerge>
                <feMergeNode/>
    ...
    // Abstract Map Blobs
    mapPattern: `
        <pattern id="map" width="400" height="400" patternUnits="userSpaceOnUse">
            <path d="M50,50 Q100,0 150,50 T250,50 T350,50" stroke="${BRAND.colors.water}" stroke-width="2" fill="none" opacity="0.3"/>
            <path d="M0,200 Q100,150 200,200 T400,200" stroke="${BRAND.colors.water}" stroke-width="2" fill="none" opacity="0.3"/>
            <circle cx="100" cy="300" r="40" fill="${BRAND.colors.water}" opacity="0.2"/>
            <circle cx="300" cy="100" r="20" fill="${BRAND.colors.water}" opacity="0.2"/>
        </pattern>
    `
};
...
function generateVoxThumbnail() {
    const width = 1280;
    const height = 720;
    const cx = width / 2;
    const cy = height / 2;
    
    // V6: The "Explained" Collage
    createSVG({
        filename: 'thumb_v6_vox.svg',
        width, height,
        content: `
            ...
            <rect width="${width}" height="${height}" fill="url(#map)"/>
            
            <!-- Diagonal Split / Paper Tear -->
            <path d="M${width*0.4},0 L${width},0 L${width},${height} L${width*0.2},${height} Z" fill="#e2e8f0" opacity="0.8"/>
            
            <!-- Central Sticker Subject (The Logo) -->
            <g transform="translate(${cx - 100}, ${cy - 100}) scale(0.5)" filter="url(#sticker)">
                 <!-- Convert logo to solid black for sticker style -->
                 <path d="${LOGO_Path}" fill="${BRAND.colors.text}"/>
            </g>
            
            <!-- Highlight Text Blocks -->
            <!-- The "Vox" style uses colored backgrounds for text -->
            <g transform="translate(60, 100)">
                <rect x="-10" y="-60" width="550" height="90" fill="${BRAND.colors.kenichiHighlight}" transform="rotate(-2)"/>
                <text x="0" y="0" fill="white" font-family="${BRAND.fonts.condensed}" font-weight="900" font-size="90" transform="rotate(-2)">
                    THE FUTURE
                </text>
            </g>
            
            <g transform="translate(60, 200)">
                 <rect x="-10" y="-60" width="600" height="90" fill="${BRAND.colors.text}" transform="rotate(-1)"/>
                 <text x="0" y="0" fill="white" font-family="${BRAND.fonts.condensed}" font-weight="900" font-size="90" transform="rotate(-1)">
                    OF INTERFACES
                </text>
            </g>
            
            <!-- Bottom Label -->
            <text x="${width - 40}" y="${height - 40}" text-anchor="end" fill="${BRAND.colors.text}" font-family="${BRAND.fonts.condensed}" font-weight="bold" font-size="40" opacity="0.6">
                EXPLAINED
            </text>
        `
    });
...
// --- Run ---
console.log('Generating Vox Style Asset...');
generateVoxThumbnail();
console.log('Done.');
